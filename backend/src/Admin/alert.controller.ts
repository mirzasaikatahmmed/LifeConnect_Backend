...
export class AlertController {
  ...
  async createAlert(
    ...
    const alert = await this.alertService.createAlert(req.user.sub, createAlertDto);
    return this.formatAlertResponse(alert);
  }

  @Get()
  async getAllAlerts(@Query('status') status?: string): Promise<AlertResponseDto[]> {
    let alerts;
    if (status === 'active') {
  ...
  async updateAlert(
    ...
    const alert = await this.alertService.updateAlert(id, req.user.sub, updateAlertDto);
    return this.formatAlertResponse(alert);
  }
  ...
  async archiveExpiredAlerts(): Promise<{ message: string; archivedCount: number }> {
