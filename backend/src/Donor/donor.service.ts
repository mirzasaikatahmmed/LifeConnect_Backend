...
export class DonorService {
    constructor(
        ...
        @InjectRepository(BloodRequest) private requestRepo: Repository<BloodRequest>,
    ...
    async register(dto: DonorRegisterDto) {
        const exists = await this.donorRepo.findOne({ where: { email: dto.email } });
    ...
    async login(dto: DonorLoginDto) {
        ...
        const token = this.signToken({ sub: donor.id, email: donor.email, role: 'donor' });
    ...
    async updateProfile(donorId: number, dto: DonorUpdateDto) {
        ...
        if (dto.lastDonationDate) donor.lastDonationDate = new Date(dto.lastDonationDate);
        if (typeof dto.isAvailable === 'boolean') donor.isAvailable = dto.isAvailable;
    ...
    async availability(donorId: number, dto: DonorAvailabilityDto) {
        await this.donorRepo.update({ id: donorId }, { isAvailable: dto.isAvailable });
        return { success: true };
    }
